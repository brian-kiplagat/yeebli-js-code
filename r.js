...
Promise.all([
  ...
  // All dependencies loaded — start form logic
  (function () {
    ...
    const loadEventDates = (memberships) => {
      ...
      upcoming.forEach((m) => {
        ...
        const label = m.price === 0
          ? `Free - ${m.name} ${date.toLocaleString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
              hour: "numeric",
          ...
          : `£${m.price} ${m.name} ${date.toLocaleString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
              hour: "numeric",
      ...
      if (phoneInput) {
        iti = window.intlTelInput(phoneInput, {
          ...
          geoIpLookup: function (success, failure) {
            fetch("https://ipinfo.io/json?token=ce7b08a16a76d5")
              .then((resp) => resp.json())
