...
Promise.all([
    ...
    (function () {
        ...
        const loadEventDates = (memberships) => {
            ...
            const noDatesMessage = document.getElementById("no-upcoming-dates");

            if (noDatesMessage) noDatesMessage.style.display = "none";

            // Clear old options & clones
            ...
            // --- 1) Populate <select> (no dedupe, use original memberships) ---
            ...
            upcomingMemberships.forEach((m) => {
                const ts = parseInt(m.dates[0].date) * 1000;
                ...
                select.add(new Option(label, m.id));
            });

            // --- 2) Populate upcoming_dates (deduped at minute level) ---
            const allDates = memberships.flatMap(m =>
                m.dates.map(d => ({ ts: parseInt(d.date) * 1000 }))
            );

            const uniqueMinutes = new Set();
            const upcomingUnique = allDates
                .filter(d => d.ts > now)
                .sort((a, b) => a.ts - b.ts)
                .filter(d => {
                    const minuteKey = Math.floor(d.ts / 60000);
                    if (uniqueMinutes.has(minuteKey)) return false;
                    uniqueMinutes.add(minuteKey);
                    return true;
                });

            if (upcomingUnique.length === 0) {
                if (noDatesMessage) noDatesMessage.style.display = "block";
                if (template) template.remove();
                return;
            }

            upcomingUnique.forEach(({ ts }) => {
                const date = new Date(ts);
                const dateText = date.toLocaleString("en-US", {
                    month: "long",
                    day: "numeric",
        ...
        window.addEventListener("load", function () {
            ...
            if (form) {
                form.addEventListener("submit", async function (e) {
                    ...
                    const nameInput = document.getElementById("lead_form_name");
                    const emailInput = document.getElementById("lead_form_email");
                    const phoneInput = document.getElementById("lead_form_phone");
                    ...
                    // Validate fields
                    if (!selectedMembership) return showFormError("Please select a date.");
                    if (!name) return showFormError("Please enter your name.");
                    if (!email || !/^\S+@\S+\.\S+$/.test(email)) return showFormError("Please enter a valid email.");
                    if (!phone) return showFormError("Please enter your phone number.");
                    if (isNaN(captchaAnswer)) return showFormError("Please answer the security question.");
                    if (captchaAnswer !== correctAnswer) {
                        showFormError("❌ Wrong answer. Please try again!");
                        generateCaptcha();
                        return;
                    }
