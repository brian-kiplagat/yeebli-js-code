    ...
    (function () {
        ...
        const loadEventDates = (memberships) => {
            ...
            const upcoming = memberships
                .flatMap(m => m.dates.map(d => ({ ...m, date: d.date }))) // support all dates
                .filter(m => parseInt(m.date || "0") * 1000 > now)
            ...
            upcoming.forEach((m) => {
                const ts = parseInt(m.date) * 1000;
                const minuteKey = Math.floor(ts / 60000); // ⬅️ normalize to minute

                if (seenMinutes.has(minuteKey)) return; // skip duplicates
                ...
                const dateText = date.toLocaleString("en-US", {
                    ...
                    year: "numeric",
                    hour: "numeric",
                    minute: "2-digit", // ensure "7:00" not "7:0"
                    hour12: true,
                });
